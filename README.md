...
│                           #   now deduplicated into one parameterized scraper
...
│                           #   Pydantic responses, structured JSON logging
├── models/
...
├── .github/workflows/ci.yml  # Lint (ruff) + test+coverage (pytest-cov,
│                              #   fails under 60%) + dependency audit (pip-audit)
...
coverage drops below 60%. See `CONTRIBUTING.md` for setup details.
